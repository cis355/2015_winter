...
function downloadFile()
{
	...
	$query = "SELECT name, size, content, type FROM ".tableName." where id=$id"; 
...
function listImages()
{
	...
	//the query to execute
	$query = "SELECT id, name, size FROM ".tableName; 
	//execute the query
	$result = $conn->query($query); 
	//read each row of data returned
	echo "<body><form action='file_delete.php' method='post' enctype='multipart/form-data'>";
	while($row = $result->fetch_assoc())
	{
		...
		echo "<a href='file_download.php?id=$id'>
		   $name - $size</a><input name='".$id."' 
           type='submit' id='delete' value=' Delete '><br>";
	}
    echo "</form></body>";	
}
